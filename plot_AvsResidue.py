...
class DatasetXvsY(project_util.DatasetXvsY):
    ...
    def get_replica_data(self):
        ...
        for i in range(Ai_vs_Q.shape[1]):
            ...
            C[self.non_pairs[i,0], self.non_pairs[i,1]] = Ai_vs_Q[Q_bin_idx, i]
...
if __name__ == "__main__":
    ...
    AvsRes_plotspecs = {"xlabel":"Residue index", "ylabel":"A", "title":"{} Non-native contacts",
            "saveas":saveas,
            "grid_dims":grid_dims,
            "coloridxs":coloridxs, "xytext":xytext, "ylims":ylims,
            "ylog":ylog}

    AvsRes_grid_plotspecs = {"xlabel":"Residue index", "ylabel":"A",
            "title":"Non-native contacts per residue",
            "saveas":"repavg_grid_AvsIndex",
            "grid_dims":(2,3),
            "coloridxs":coloridxs, "xytext":xytext, "ylims":(0,0.08), "xlims":(0,1)}

    AvsRes_raw_data = DatasetXvsY(topologies, top_names, b_values, 
                replicas, datapath_x, datapath_y)

    AvsRes_data = project_util.DatasetXvsY(topologies, top_names, b_values, replicas,"","", empty=True)
    for t in range(len(topologies)):
        for n in range(len(top_names[t])):
            for b in range(len(b_values)):
                for rep in range(len(AvsRes_raw_data.ydata[t][n][b])):
                    AvsRes_data.ydata[t][n][b].append(AvsRes_raw_data.ydata[t][n][b][rep])
                    AvsRes_data.xdata[t][n][b].append(AvsRes_raw_data.xdata[t][n][b][rep]/float(prot_sizes[t][n]))
    AvsRes_data._calc_repavg()

    project_plotter.plot_repavg(AvsRes_data, AvsRes_plotspecs)
    project_plotter.plot_repavg_protein_grid(AvsRes_data, AvsRes_grid_plotspecs)
