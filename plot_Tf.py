...
if __name__ == "__main__":
    ...
    Tf_over_Tg_plotspecs = {"xlabel":"Frustration ($b$)", "ylabel":r"$\frac{T_f}{T_g}$", 
            "legend_loc":1, "saveas":"Tf_over_Tg", "ylabel_rot":"vertical"}
    Tf_over_Tg_plotspecs.update(plotstyle)
    ...
    Tf_over_Tg_data = project_util.Dataset(topologies, top_names, b_values, replicas, "", empty=True)
    for t in range(len(topologies)):
        for n in range(len(top_names[t])):
            for b in range(len(b_values)):
                Tf_b = Tf_data.data[t][n][b]
                ...
                for rep in range(len(Tf_b)):
                    if not np.isnan(Tf_b[rep]) and not np.isnan(Tg_b[rep]):
                        Tf_over_Tg_data.data[t][n][b][rep] = Tf_b[rep]/Tg_b[rep]
    Tf_over_Tg_data._calc_repavg()
        ...
        for n in range(len(top_names[t])):
            ...
            for b in range(len(b_values)):
                ...
                if (np.sum(good1) > 1) and (np.sum(good2) > 1):
                    ...
                    temp_std.append(np.nan)
            Tf_over_Tg_data.stddata[t][n] = np.array(temp_std)



    project_plotter.plot_data(Tf_data, Tf_plotspecs)
    project_plotter.plot_data(Tg_data, Tg_plotspecs)
    project_plotter.plot_data(Tg_over_Tf_data, Tg_over_Tf_plotspecs)
    project_plotter.plot_data(Tf_over_Tg_data, Tf_over_Tg_plotspecs)
    plt.show()
